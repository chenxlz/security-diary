...
<iframe src="/widgets/DynamicTodo" data-src="/widgets/DynamicTodo" data-subtype="widget" border="0" frameborder="no" framespacing="0" allowfullscreen="true" style="width: 1525px; height: 550px;"></iframe>

## 👮我是个保安

<iframe src="https://cn.widgetstore.net/view/index.html?q=0a4ec1f962b061f30b9e70011eb1b10b.e22ab3646471fcb0000b7c2f7fefe3fe" data-src="" border="0" frameborder="no" framespacing="0" allowfullscreen="true" style="width: 1679px; height: 201px;"></iframe>
...
* ​`ASCII`​(American Standard Code for Information Interchange)：美国信息交换标准代码
  ...
  ```js
  在所有字符集中，最知名的可能要数被称为 ASCII 的8 位字符了。
...
* ​`UTF-8`​：最常用
  ...
  ```js
  UTF-8（8 位元，Universal Character Set/Unicode Transformation Format）是针对 Unicode 的一种可变长度字符编码，也叫万国码、统一码。它可以用来表示 Unicode 标准中的任何字符，而且其编码中的第一个字节仍<mark>与 ASCII 相容</mark>，使得原来处理 ASCII 字符的软件无须或只进行少部分修改后，便可继续使用。
  ```
* ​`UTF-16`​
...
1. forEach中无法使用async、await来实现同步请求
2. forEach循环时，无法给简单类型数据赋值
...
在使用table中的`append`​插槽时，因为里面也是嵌套的一个表格，也有分页器，所以希望保留分页器的数据，后面发现append的每次展开都会创建新的dom，这导致每次的分页器数据不会保存。

1. 每次的展开或折叠都会造成组件的创建和销毁
2. **组件存在不销毁的情况：使用了全局变量，导致js的内存回收机制不生效，而使组件虽然被重新创建而老的组件并没有销毁**。垃圾回收机制。如使用定时器，这个是windows下的，所以会让组件一直无法被回收。
3. 怎么解决分页器的数据呢？暂时没有思路，或许可以使用本地缓存来保存变量
...
核心：((20220926111147-5dmmgr9 '路由传参'))
