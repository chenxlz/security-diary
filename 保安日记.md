...
<iframe src="https://cn.widgetstore.net/view/index.html?q=0a4ec1f962b061f30b9e70011eb1b10b.e22ab3646471fcb0000b7c2f7fefe3fe" data-src="" border="0" frameborder="no" framespacing="0" allowfullscreen="true" style="width: 1601px; height: 150px;"></iframe>
...
* 绝对路径：绝对路径其实就是指从根目录开始的完整路径，它可以唯一确定一个文件或目录的位置。在开发中，**绝对路径通常以“/”开头**
  ...
  ```
* 相对路径：指相对于当前文件中的路径，它可以简化路径的书写，但需要注意路径的正确性，一般相对路径通常有两种表示方式：相对于当前文件的路径和相对于网站根目录的路径。
...
请求器发起每次发起请求时，会把`cookie`​携带过去给服务器，`cookie`​中一般包含了用户的信息，所以`cookie`​能够解决`http`​无状态的问题。

​`cookie`​本地查看的时候一串字符串，存储的信息是`name=value`​的信息，多个`cookie`​之间通过` ; `​来区分。

​`cookie`​也有属于它的属性，可以设置`cookie`​所实用的域名、`cookie`​过期时间、`cookie`​是否可以使用js来获取、`cookie`​是否可以第三方携带。

简单的理解：`cookie`​用来保存后端返回的用户信息，来识别用户身份，通过设置属性来限制`cookie`​的使用。

​![image](assets/image-20230514150520-zpl0dwv.png)​

**定义：**​`cookie`​就是一段数据，在浏览器后台查看时就是一串字符串，里面保存了一些信息。存储是以 `Name=Value`​​ 的形式。
...
如果是跨域请求，cookie默认是不携带的。如果我们设置需要请求中携带cookie的话，需要满足以下条件：

1. cookie中的`SameSite`​属性设置成None，同时`Secure`​要为true，即只能是https请求。
2. 这时候浏览器会发起预检请求，服务器要告诉浏览器这个请求允许的请求方法、和发起远程连接，满足这些条件，才能把cookie携带过去。
