...
​`cookie`​除了本地`js`​可以设置之外，每次发起请求之后，后端返回也可以设置`cookie`​保存在浏览器中，可以通过请求头里的`set-cookie`​进行查看，请求器发起每次发起请求时，会把`cookie`​携带过去给服务器，`cookie`​中一般包含了用户的信息，所以`cookie`​能够解决`http`​无状态的问题
